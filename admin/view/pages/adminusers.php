        ...
        <?php foreach ($users as $user): ?>
            <tr class="bg-neutral-primary border-b border-default text-white">
                ...
                <td class="flex gap-4 my-2 px-6 py-4">
                    <a href="<?= BASE_URL ?>admin/utilisateurs/edit/<?= $user['id'] ?>" class="btn btn-primary rounded font-bold border-none bg-[#1576e2] text-white py-2 px-4 hover:opacity-80">
                        Modifier
                    </a>
                    <a href="<?= BASE_URL ?>admin/utilisateurs/delete/<?= $user['id'] ?>" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet utilisateur ?');" class="btn btn-primary rounded font-bold border-none bg-red-600 text-white py-2 px-4 hover:opacity-80">
                        Supprimer
                    </a>
                </td>
            </tr>
...
<a href="<?= BASE_URL ?>admin/utilisateurs/create" class="w-fit flex items-center gap-3 px-6 py-5 my-5 bg-[#E0F2FE] text-[#2C4A63] font-semibold rounded-xl border border-[#7AAACE]/30 hover:bg-[#7AAACE] hover:text-white transition-all shadow-sm">
